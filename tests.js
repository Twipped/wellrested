...
test('request factories', (t) => {
  ...
  t.only('invoking client directly', async (t) => {
    ...
    t.same(res.body, { ok: true }, 'Got back the nocked response');
  });
  ...
  t.test('invoking client method', async (t) => {
    ...
    t.same(body, { ok: true }, 'Got back the nocked response');
  });
  ...
  t.test('invoking endpoint client', async (t) => {
    ...
    t.same(await client.user('get', { username: 'USERNAME' }), { ok: true }, 'Got back the nocked response');
  });
  ...
  t.test('invoking endpoint method', async (t) => {
    ...
    t.same(await client.message.post(), { ok: true }, 'Got back the nocked response');
  });
  ...
  t.test('invoking a mixin', async (t) => {
    ...
    t.same(await client.getUser('USERNAME'), { ok: true }, 'Got back the nocked response');
  });

  t.end();
});

test('data encoding', (t) => {
  ...
  t.test('performs a request with the correct basic auth', async (t) => {
    ...
    t.same(await client.lookup.get().query({ value: '123 1/2' }), { ok: true }, 'Got back the nocked response');
  });
...
test('basic auth', (t) => {
  ...
  t.test('performs a request with the correct basic auth', async (t) => {
    ...
    t.same(await client.put('/user/USERNAME'), { ok: true }, 'Got back the nocked response');
  });
...
test('bearer token', (t) => {
  ...
  t.test('performs a request with the correct auth token', async (t) => {
    ...
    t.same(await client.patch('/user/USERNAME'), { ok: true }, 'Got back the nocked response');
  });
...
test('deep endpoints and mixins', (t) => {
  ...
  t.test('deep endpoints, nested mixins', async (t) => {
    ...
    t.same(await client.user.getUser('USERNAME'), { ok: true }, 'Got back the nocked response');
  });
  ...
  t.test('deep endpoints, nested mixins, part 2', async (t) => {
    ...
    t.same(await client.user.getMessages('USERNAME'), { ok: true }, 'Got back the nocked response');
  });
  ...
  t.test('nested endpoints, deep mixins', async (t) => {
    ...
    t.same(await client.orders.getByType('TYPE'), { ok: true }, 'Got back the nocked response');
  });
